...
def parse_invoice_text(text, is_credit_note=False):
    ...
    if is_credit_note:
        ...
        # Look for the number that appears in the table row (the line item total)
        # Pattern: Description then number (unit price) then number (total)
        match = re.search(r'Refund.*?(\d+(?:\.\d{2})?)\s*(\d+(?:\.\d{2})?)', text, re.DOTALL)
        if match:
            # The second number is the total amount (4000.00)
            result["amount"] = match.group(2).replace(',', '')
        else:
            # Fallback: find the number that is NOT the VAT (600) and NOT the total credit (4600)
            numbers = re.findall(r'(\d+(?:\.\d{2})?)', text)
            for num in numbers:
                val = float(num.replace(',', ''))
                # Skip VAT (600) and total credit (4600)
                if val == 600.00 or val == 4600.00:
                    continue
                # Accept the first reasonable number (should be 4000)
                if 10 < val < 10000:
                    result["amount"] = num
                    break
    else:
        ...
        if amount_match:
            ...
            amount_match = re.search(r'Amount\s+Due:\s*[R$]?\s*(\d+(?:\.\d{2})?)', text, re.IGNORECASE)
            if amount_match:
                result["amount"] = amount_match.group(1).replace(',', '')
