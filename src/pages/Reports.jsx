...
export default function Reports() {
  ...
  const totalPending = filteredDocs.filter(
    (d) => d.status !== "approved" && d.status !== "rejected",
  ).length;
  const totalRejected = filteredDocs.filter((d) => d.status === "rejected").length;
  const totalAmount = filteredDocs
    .filter((d) => d.status === "approved")
    .reduce((sum, d) => sum + parseFloat(d.amount || 0), 0);
  const totalVAT = filteredDocs
    .filter((d) => d.status === "approved")
    .reduce((sum, d) => sum + parseFloat(d.vat || 0), 0);
  ...
  const invoiceCount = filteredDocs.filter(
    (d) => d.document_type === "invoice",
  ).length;
  const creditNoteCount = filteredDocs.filter(
  ...
  const spendSummaryByVendor = Object.entries(vendorData)
    .map(([vendor, amount]) => ({ vendor, amount: amount.toFixed(2) }))
    .sort((a, b) => parseFloat(b.amount) - parseFloat(a.amount));

  // VAT report data
  const vatReportData = filteredDocs
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredDocs.map((doc) => [
      ...
      parseFloat(doc.vat || 0).toFixed(2),
      doc.status,
      doc.document_type,
    ]);
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "24px" }}>
        ...
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "16px", border: "1px solid #e5e7eb" }}>
          <p style={{ fontSize: "12px", color: "#6b7280" }}>Total Value (Approved)</p>
          <p style={{ fontSize: "20px", fontWeight: "bold", color: "#1f2937" }}>R {loading ? "..." : totalAmount.toLocaleString()}</p>
        </div>
      </div>

      {/* Filters */}
      <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "16px", marginBottom: "24px", border: "1px solid #e5e7eb" }}>
        ...
        <div style={{ display: "flex", gap: "16px", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <input type="date" value={filterStartDate} onChange={(e) => setFilterStartDate(e.target.value)} placeholder="Start Date" style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid #d1d5db" }} />
          <span style={{ color: "#6b7280" }}>to</span>
          <input type="date" value={filterEndDate} onChange={(e) => setFilterEndDate(e.target.value)} placeholder="End Date" style={{ padding: "8px 12px", borderRadius: "8px", border: "1px solid #d1d5db" }} />

          <button onClick={clearFilters} style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid #d1d5db", backgroundColor: "#f3f4f6", cursor: "pointer" }}>Clear Filters</button>
          ...
          <div style={{ marginLeft: "auto", fontSize: "12px", color: "#6b7280" }}>Showing {filteredDocs.length} of {documents.length} documents</div>
        </div>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #e5e7eb" }}>
                <th style={{ textAlign: "left", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Vendor</th>
                <th style={{ textAlign: "right", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Total Spend (R)</th>
              </tr>
            </thead>
            <tbody>
              {spendSummaryByVendor.map((item) => (
                <tr key={item.vendor} style={{ borderBottom: "1px solid #f3f4f6" }}>
                  <td style={{ padding: "8px", color: "#1f2937" }}>{item.vendor}</td>
                  <td style={{ padding: "8px", textAlign: "right", color: "#1f2937" }}>R {parseFloat(item.amount).toLocaleString()}</td>
                </tr>
              ))}
              {spendSummaryByVendor.length === 0 && (
                <tr><td colSpan="2" style={{ padding: "20px", textAlign: "center", color: "#9ca3af" }}>No data available</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showVATReport && (
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "20px", marginBottom: "24px", border: "1px solid #e5e7eb" }}>
          ...
          <div style={{ marginBottom: "16px", display: "flex", gap: "16px", flexWrap: "wrap" }}>
            <div style={{ backgroundColor: "#f0fdf4", padding: "12px", borderRadius: "8px", flex: 1 }}>
              <p style={{ fontSize: "12px", color: "#6b7280" }}>Total VAT Collected</p>
              <p style={{ fontSize: "24px", fontWeight: "bold", color: "#10b981" }}>R {totalVAT.toLocaleString()}</p>
            </div>
            <div style={{ backgroundColor: "#eff6ff", padding: "12px", borderRadius: "8px", flex: 1 }}>
              <p style={{ fontSize: "12px", color: "#6b7280" }}>Total Taxable Amount</p>
              <p style={{ fontSize: "24px", fontWeight: "bold", color: "#3b82f6" }}>R {totalAmount.toLocaleString()}</p>
            </div>
            <div style={{ backgroundColor: "#fef3c7", padding: "12px", borderRadius: "8px", flex: 1 }}>
              <p style={{ fontSize: "12px", color: "#6b7280" }}>Average VAT Rate</p>
              <p style={{ fontSize: "24px", fontWeight: "bold", color: "#f59e0b" }}>{totalAmount > 0 ? ((totalVAT / totalAmount) * 100).toFixed(1) : 0}%</p>
            </div>
          </div>
          <div style={{ overflowX: "auto", maxHeight: "400px" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead style={{ position: "sticky", top: 0, backgroundColor: "white" }}>
                <tr style={{ borderBottom: "1px solid #e5e7eb" }}>
                  <th style={{ textAlign: "left", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Vendor</th>
                  <th style={{ textAlign: "left", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Invoice #</th>
                  <th style={{ textAlign: "left", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Date</th>
                  <th style={{ textAlign: "right", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Amount (R)</th>
                  <th style={{ textAlign: "right", padding: "8px", fontSize: "12px", color: "#6b7280" }}>VAT (R)</th>
                  <th style={{ textAlign: "right", padding: "8px", fontSize: "12px", color: "#6b7280" }}>Total (R)</th>
                </tr>
              </thead>
              <tbody>
                {vatReportData.map((doc, idx) => (
                  <tr key={idx} style={{ borderBottom: "1px solid #f3f4f6" }}>
                    <td style={{ padding: "8px", color: "#1f2937" }}>{doc.vendor}</td>
                    <td style={{ padding: "8px", color: "#1f2937" }}>{doc.invoice_number}</td>
                    <td style={{ padding: "8px", color: "#1f2937" }}>{doc.date}</td>
                    <td style={{ padding: "8px", textAlign: "right", color: "#1f2937" }}>R {parseFloat(doc.amount).toLocaleString()}</td>
                    <td style={{ padding: "8px", textAlign: "right", color: "#1f2937" }}>R {parseFloat(doc.vat).toLocaleString()}</td>
                    <td style={{ padding: "8px", textAlign: "right", color: "#1f2937" }}>R {parseFloat(doc.total).toLocaleString()}</td>
                  </tr>
                ))}
                {vatReportData.length === 0 && (
                  <tr><td colSpan="6" style={{ padding: "20px", textAlign: "center", color: "#9ca3af" }}>No approved documents with VAT data</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "24px", marginBottom: "24px" }}>
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "20px", border: "1px solid #e5e7eb" }}>
          ...
          {loading ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>Loading...</p>) : statusData.length === 0 ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>No data available</p>) : (
        ...
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "20px", border: "1px solid #e5e7eb" }}>
          ...
          {loading ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>Loading...</p>) : typeData.length === 0 ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>No data available</p>) : (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "24px", marginBottom: "24px" }}>
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "20px", border: "1px solid #e5e7eb" }}>
          ...
          {loading ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>Loading...</p>) : monthlyChartData.length === 0 ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>No data available</p>) : (
            <ResponsiveContainer width="100%" height={300}>
        ...
        <div style={{ backgroundColor: "white", borderRadius: "12px", padding: "20px", border: "1px solid #e5e7eb" }}>
          ...
          {loading ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>Loading...</p>) : vendorChartData.length === 0 ? (<p style={{ textAlign: "center", color: "#9ca3af", padding: "40px" }}>No data available</p>) : (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={vendorChartData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" tickFormatter={(value) => `R${value.toLocaleString()}`} />
                <YAxis type="category" dataKey="name" width={100} />
                <Tooltip formatter={(value) => [`R ${value.toLocaleString()}`, "Total Spend"]} />
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead style={{ backgroundColor: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
            <tr>
              ...
              <th style={{ padding: "12px", textAlign: "left", fontSize: "12px", color: "#6b7280" }}>Type</th>
              <th style={{ padding: "12px", textAlign: "left", fontSize: "12px", color: "#6b7280" }}>Vendor</th>
              <th style={{ padding: "12px", textAlign: "left", fontSize: "12px", color: "#6b7280" }}>Document #</th>
              <th style={{ padding: "12px", textAlign: "left", fontSize: "12px", color: "#6b7280" }}>Date</th>
              <th style={{ padding: "12px", textAlign: "right", fontSize: "12px", color: "#6b7280" }}>Amount</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="7" style={{ padding: "48px", textAlign: "center", color: "#9ca3af" }}>No documents found</td></tr>
            ) : (
                  ...
                  <React.Fragment key={doc.id}>
                    <tr style={{ borderBottom: "1px solid #f3f4f6", cursor: "pointer" }}>
                      <td style={{ padding: "12px", textAlign: "center" }}>
                        <button onClick={() => toggleRow(doc.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "16px" }}>{expandedRow === doc.id ? "▼" : "▶"}</button>
                      </td>
                      <td style={{ padding: "12px" }}>{doc.document_type === "invoice" ? "📄 Invoice" : "📝 Credit Note"}</td>
                      <td style={{ padding: "12px", color: "#1f2937" }}>{doc.vendor_name}</td>
                      <td style={{ padding: "12px", color: "#1f2937" }}>{doc.invoice_number}</td>
                      <td style={{ padding: "12px", color: "#1f2937" }}>{doc.date}</td>
                      <td style={{ padding: "12px", textAlign: "right", color: "#1f2937" }}>R {parseFloat(doc.amount).toLocaleString()}</td>
                      <td style={{ padding: "12px", textAlign: "center" }}>
                        <span style={{ fontSize: "11px", padding: "4px 8px", borderRadius: "12px", backgroundColor: doc.status === "approved" ? "#d1fae5" : doc.status === "rejected" ? "#fee2e2" : "#fef3c7", color: doc.status === "approved" ? "#065f46" : doc.status === "rejected" ? "#991b1b" : "#92400e" }}>
                    ...
                    {expandedRow === doc.id && (
                      <tr>
                        <td colSpan="7" style={{ padding: "16px", backgroundColor: "#f9fafb" }}>
                          <div style={{ borderLeft: "3px solid #e5e7eb", paddingLeft: "20px", margin: "8px 0" }}>
                            <h4 style={{ fontSize: "14px", fontWeight: "600", marginBottom: "12px", color: "#1f2937" }}>Approval Timeline</h4>
                              ...
                              {[1, 2, 3].map((step) => {
                                ...
                                return (
                                  <div key={step} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                                    <div style={{ width: "120px", fontSize: "13px", color: "#6b7280" }}>Step {step}: {getStepName(step)}</div>
                                    <div style={{ flex: 1 }}>
                                      {stepStatus.status === "approved" && (<span style={{ color: "#10b981", fontSize: "13px" }}>✅ Approved {stepStatus.by && `by ${stepStatus.by.substring(0, 8)}`}{stepStatus.at && ` on ${formatDate(stepStatus.at)}`}</span>)}
                                      {stepStatus.status === "rejected" && (<span style={{ color: "#ef4444", fontSize: "13px" }}>❌ Rejected {stepStatus.by && `by ${stepStatus.by.substring(0, 8)}`}{stepStatus.at && ` on ${formatDate(stepStatus.at)}`}</span>)}
                                      {stepStatus.status === "pending" && (<span style={{ color: "#f59e0b", fontSize: "13px" }}>⏳ Pending</span>)}
                                      {stepStatus.status === "blocked" && (<span style={{ color: "#6b7280", fontSize: "13px" }}>🚫 Blocked (rejected at earlier step)</span>)}
                                    </div>
                                  </div>
                                );
                              })}
                            ...
                            {approvalsList && approvalsList.length > 0 && (
                              <div style={{ marginTop: "12px", fontSize: "11px", color: "#9ca3af" }}>Last updated: {formatDate(approvalsList[approvalsList.length - 1]?.approved_at)}</div>
                            )}
                          </div>
                        </td>
                      </tr>
